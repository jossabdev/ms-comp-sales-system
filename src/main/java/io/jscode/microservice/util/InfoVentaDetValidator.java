...
public class InfoVentaDetValidator {
	...
	public InfoVentaDetDTO validarActualizarInfoVentaDet(InfoVentaDetDTO request, Map<String, String> headers)
		...
		InfoVentaDetService infoVentaDetService = (InfoVentaDetServiceImpl) beanFactory
				.getBean(infoVentaDetServiceimpl);
		...
		if (detalleVentaExistente == null) {
			throw new ExcepcionGenerica(
					"El objeto detalleVenta con idVentaDet " + request.getIdVentaDet() + " no existe", 404);
		}
	...
	public void validarGuardarDetalleVenta(InfoVentaDetDTO request, Map<String, String> headers)
			throws ExcepcionGenerica {
		if (request.getVenta() == null || request.getVenta().getIdVenta() == null) {
	...
	public InfoVentaDetDTO validarEliminarVentaDet(InfoVentaDetDTO request, Map<String, String> headers)
			throws ExcepcionGenerica {

		if (request.getIdVentaDet() == null) {
			throw new ExcepcionGenerica("El parametro idVentaDe es requerido");
		}
		...
		// se consulta venta detalle por filtro
		InfoVentaDetService infoVentaDetService = (InfoVentaDetServiceImpl) beanFactory.getBean(infoVentaDetServiceimpl);
		InfoVentaDetDTO detalleVentaExistente;
		try {
			detalleVentaExistente = infoVentaDetService.obtenerDetalleVentaPor(request);

			detalleVentaExistente.setFeUltMod(LocalDateTime.now());

			if (detalleVentaExistente.getUsrUltMod() == null || detalleVentaExistente.getUsrUltMod().isBlank()) {
				String usrUltMod = headers.get("user");

				if (usrUltMod == null || usrUltMod.isBlank()) {
					throw new ExcepcionGenerica("El parametro header user es requerido");
				}
				detalleVentaExistente.setUsrUltMod(usrUltMod);
			}
			detalleVentaExistente.setIpUltMod(salesUtils.getClientIp());
		} catch (NoSuchElementException e) {
			e.printStackTrace();
			throw new ExcepcionGenerica("La venta no existe. Detalle de error: " + e.getMessage(), 404);
		}
		return detalleVentaExistente;
	}
}
